...
def edit_answer(answer_id, question_id):
    ...
    if request.method == "GET":
        return render_template(
            "post_answer.html", question_id=question_id, message=old_message
        )
    elif request.method == "POST":
        new_message = request.form.get("message")
        data_manager.edit_answer_to_question(
            answer_id, old_message, new_message, user_id
        )
        return redirect(url_for("get_entry", question_id=question_id))
...
def add_comment(answer_id, question_id):
    if request.method == "GET":
        ...
        data_manager.add_comment_to_answer(
            answer_id, question_id, comment_message, username, user_id
        )
        return redirect(url_for("get_entry", question_id=question_id))
...
def add_comment_question(question_id):
    if request.method == "POST":
        ...
        data_manager.inject_question_comment(
            question_id, comment_message, username, user_id
        )
...
def edit_comment(comment_id, question_id):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("get_entry", question_id=question_id))
    return render_template(
        "edit_comment.html", comment_id=comment_id, comments=this_comment
    )
...
def show_user_details():
    ...
    return render_template(
        "user_details.html", username=user_name, user_details=user_details
    )
